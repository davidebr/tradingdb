...
class TestStockORM(unittest.TestCase):
    ...
    def delete_bogus_currency(self):
        with self.session.begin() as mysession:
            ret = (
                mysession.query(stock.Currency)
                .filter(stock.Currency.name == BOGUS_CUR)
                .one()
            )
    ...
    def delete_bogus_exchange(self):
        ...
        with self.session.begin() as mysession:
            ret = (
                mysession.query(stock.Exchange)
                .filter(stock.Exchange.name == BOGUS_XC)
                .one()
            )
            mysession.delete(ret)
            mysession.commit()
    ...
    def create_bogus_stock(self):
        with self.session.begin() as mysession:
            eid = (
                mysession.query(stock.Exchange.id)
                .filter(stock.Exchange.name == BOGUS_XC)
                .one()[0]
            )
            cid = (
                mysession.query(stock.Currency.id)
                .filter(stock.Currency.name == BOGUS_CUR)
                .one()[0]
            )
    ...
    def delete_bogus_stock(self):
        ...
        with self.session.begin() as mysession:
            ret = (
                mysession.query(stock.Stock)
                .filter(stock.Stock.company == "bogusst")
                .one()
            )
            mysession.delete(ret)
            mysession.commit()
    ...
    def add_data_to_symbol(self, symbol, df, interval):
        ...
        if interval not in valid_intervals:
            raise ValueError(
                "interval " + interval + "not in " + ",".join(valid_intervals)
            )
        try:
            with self.session.begin() as mysession:
                sid = (
                    mysession.query(stock.Stock.id)
                    .filter(stock.Stock.symbol == symbol)
                    .one()[0]
                )
                pid = (
                    mysession.query(stock.Period.id)
                    .filter(stock.Period.name == interval)
                    .one()[0]
                )
    ...
    def test_create_stock(self):
        ...
        with self.session.begin() as mysession:
            ...
            newstock = stock.Stock(
                company=BOGUS_ST, symbol=BOGUS_ST, exchange_id=eid, currency_id=cid
            )
            # now I try to add it again and should trigger an error
            with self.assertRaises(IntegrityError):
                mysession.add(newstock)
    ...
    def test_symbol_download_vod(self):
        ...
        if OFFLINE:
            ...
            df = finance_tools.download_data_from_symbols(
                ["VOD"], time_start, time_end, interval="1d", mypath=None
            )
