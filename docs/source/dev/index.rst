...
The TradingDB project implements a regression
...
not broken existing functionality. The TradingDB
...
``tradingdb`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``tradingdb`` package can be found.

.. code-block:: console

    (tradingdb) $ cd tests
    (tradingdb) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (tradingdb) $ python -m test_version
...
.. code-block:: console
    ...
    (tradingdb) $ make check-static-analysis
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/tradingdb/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (tradingdb) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (tradingdb) $ make dist-upload

  The package should now be available at https://pypi.org/project/tradingdb/
