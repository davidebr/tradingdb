...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt tradingdb
...
.PHONY: format
format:
	@black src/tradingdb tests examples


# help: check-format                   - check code format compliance
.PHONY: check-format
check-format:
	@black --check src/tradingdb tests examples
...
.PHONY: check-types
check-types:
	@cd src; mypy -p tradingdb --ignore-missing-imports


# help: check-lint                     - run static analysis checks
.PHONY: check-lint
check-lint:
	@pylint --rcfile=.pylintrc tradingdb ./tests setup.py ./examples
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/tradingdb*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./tradingdb-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/tradingdb-*-py3-none-any.whl
